...
function ProductPage() { // functional component
  ...
  function show(drink) {
    let button;
    if(!hasInCart(drink.id)) button = <div><button onClick={() => handleProductAdd(getProduct(drink.id))} className='bg-blue-700 mx-8 rounded-md hover:bg-blue-600 p-2'>Add</button></div>
    else button = <div><button onClick={() => handleProductAdd(getProduct(drink.id))} className='bg-blue-700 mr-2 rounded-md hover:bg-blue-600 p-2'>Add</button>
    <button onClick={() => handleProductRemove(getProduct(drink.id))} className='bg-rose-800 ml-2 rounded-md hover:bg-rose-600 p-2'>Remove</button>
    <p className='text-sm'>Already {getAmount(drink.id)}x in cart</p></div>
    return (
      <div className=' border-black imageZoom'>
        <img className='object-contain rounded-s-xl h-48' src={getImage(drink.id)}></img> 
        <p>{drink.name}</p>
        {button}
